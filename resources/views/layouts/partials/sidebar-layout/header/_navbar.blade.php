...
<div class="app-navbar flex-shrink-0">
  ...
  {{-- @if(authPartnerId())
    <div class="app-navbar-item ms-1 ms-md-3">
      <a href="{{ routed('display', [authPartner()->hash_id]) }}" class="btn btn-sm btn-light" target="_blank">Display TV</a>
    </div>
  @endif --}}
  {{--<!--begin::Activities-->
  ...
  <div class="app-navbar-item ms-1 ms-md-4" id="kt_header_user_menu_toggle">
    <div class="cursor-pointer symbol symbol-35px" data-kt-menu-trigger="{default: 'click', lg: 'hover'}" data-kt-menu-attach="parent" data-kt-menu-placement="bottom-end">
      ...
      @else
        <div class="symbol-label fs-3 {{ app(GetThemeType::class)->handle('bg-light-? text-?', auth()->user()?->name ?? 'X') }}">
          {{ substr(auth()->user()?->name ?? 'X', 0, 1) }}
        </div>
      @endif--}}
      <div class="symbol-label fs-3 {{ app(GetThemeType::class)->handle('bg-light-? text-?', auth()->user()?->name ?? 'X') }}">
        {{ substr(auth()->user()?->name ?? 'X', 0, 1) }}
