...
namespace App\KfnTables\Profile;

use App\Enums\UserType;
use App\ModelRules\Profile\ProfileMdRule;
use App\Models\User;
...
class ProfileTable extends KfnTable
{
    public ProfileMdRule $rule;
    protected string $tableId = 'profile-table';

    public function __construct(ProfileMdRule|null $rule = null)
    {
        parent::__construct();

        $this->rule = $rule instanceof ProfileMdRule
            ? $rule
            : new ProfileMdRule();
    }
    ...
    public function dataTable(QueryBuilder $query): DataTableAbstract
    {
        return kfnTableEloquent($query)
            ...
            ->addColumn('travel', fn(User $model) => $model->travel?->name ?? '')
            ...
            ->addColumn('action', function (User $model) {
                $showLink = routed('app.profile.show', $model->hash);
                $editLink = routed('app.profile.edit', $model->hash);
                $delLink = routed('app.profile.delete', $model->hash);
            ...
            ->rawColumns(['action', 'partner', 'travel', 'name', 'username', 'phone', 'email', 'status', 'type', 'created', 'updated'])
            ->setRowId('id');
    }
    ...
    public function query(User $model): QueryBuilder
    {
        $qry = $model->newQuery()
            ...
            ->where(function (QueryBuilder $qry) {
                $qry->whereNotNull('partner_id')
                    ->orWhereNotNull('travel_id');
            });

        if (authPartnerId()) {
            $qry->where('partner_id', authPartnerId() ?? null);
        }
